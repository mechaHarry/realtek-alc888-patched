...
static void alc269vb6_hp_automute_hook(struct hda_codec *codec,
					 struct hda_jack_tbl *jack)
{
	struct alc_spec *spec = codec->spec;
	int vref,i;
	...
	for (i=1; i<11; i++) {
		if (!spec->gen.hp_jack_present && 
		    (snd_hda_codec_read(codec, 0x18, 0, AC_VERB_GET_PIN_WIDGET_CONTROL, 0) & 0xf) > 0) {
			msleep(5*i);
			snd_hda_codec_write(codec, 0x18, 0, AC_VERB_SET_PIN_WIDGET_CONTROL, 0);
		}
	}
	
}

static void alc269vb6_fixup_dmic_func(struct hda_codec *codec,
				     const struct hda_fixup *fix, int action)
{
	struct alc_spec *spec = codec->spec;
	int val;
	if (action == HDA_FIXUP_ACT_PRE_PROBE) {
		spec->parse_flags |= HDA_PINCFG_HEADSET_MIC;
		spec->gen.hp_automute_hook = alc269vb6_hp_automute_hook;
	}
	val = alc_read_coef_idx(codec, 0x05);
	alc_write_coef_idx(codec, 0x05, val & ~(1 << 6));
}
...
enum {
	...
	ALC269VB6_FIXUP_DMIC,
	ALC269VB6_FIXUP_COMBO_JACK_VERB,
};
...
static const struct hda_fixup alc269_fixups[] = {
	...
	[ALC269VB6_FIXUP_DMIC] = {
		.type = HDA_FIXUP_FUNC,
		.v.func = alc269vb6_fixup_dmic_func,
	},
	[ALC269VB6_FIXUP_COMBO_JACK_VERB] = {
		.type = HDA_FIXUP_VERBS,
		.v.verbs = (const struct hda_verb[]) {
			{0x20, AC_VERB_SET_COEF_INDEX, 0x04},
			{0x20, AC_VERB_SET_PROC_COEF,  0x9827},
			{ }
		},
		.chained = true,
		.chain_id = ALC269VB6_FIXUP_DMIC
	},
};
...
static const struct snd_pci_quirk alc269_fixup_tbl[] = {
	...
	SND_PCI_QUIRK(0x1297, 0x2032, "Shuttle NB", ALC269VB6_FIXUP_COMBO_JACK_VERB),
